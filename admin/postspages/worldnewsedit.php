...
<?php 
 ...
 if(isset($_GET['edit'])){
   ...
   $imageName = "SELECT worldnewsimages FROM worldnews WHERE id = '{$idForUpdate}' ";
   $result = mysqli_query($connection,$imageName);
    while($rows = mysqli_fetch_assoc($result)){
      $selectedImageName = $rows['worldnewsimages'];
    }
 }
...
//image code end here
   ...
   if(!empty($imgName)){
       ...
       $query = "UPDATE worldnews SET worldnewstitle = '{$worldnewsTitle}', worldnewsauthor = '{$worldnewsAuthor}', worldnewsdate = now(), worldnewsimages = '{$imageChangeName}', worldnewscontent = '{$worldnewsContent}' WHERE id = '{$idForUpdate}' ";
...
if($update){
    ...
    $query = "UPDATE worldnews SET worldnewstitle = '{$worldnewsTitle}', worldnewsauthor = '{$worldnewsAuthor}', worldnewsdate = now(), worldnewscontent = '{$worldnewsContent}' WHERE id = '{$idForUpdate}' ";
 $update = mysqli_query($connection, $query);
if($update){
         echo "Updating Data, Please wait....."."&nbsp&nbsp"."<img  style='width:5%; height:5%;' src='../image/redirecting.gif'>";         
     }else{
...
$query = "SELECT * FROM worldnews WHERE id = {$idForEdit}" ;
...
<a style="color:#FFFFFF;" href="../posts/worldnews.php"><button type="button" style="float:right; padding:5px 20px 5px 20px; font-size:20px;" class="btn btn-primary">Go Back Post</button></a>
   <span class="label label-info">Your are in Edit worldnews Page </span>
   <form action="" class="form-group" method="post" enctype="multipart/form-data" name="editPosts" onsubmit="return(validateForUpdate());">
    <table class="table table-striped table-hover table-bordered">
       <th style="text-align:center;"><h3>Update worldnews Post</h3></th>
        <tr style="background:#f2f2f2;"><td><b>worldnews Title</b><p style="float:right; color:red;" id="postTitleErrorMsg"></p></td></tr>
        <tr><td><input type="text" class="form-control" name="postTitle" value="<?php echo $db_worldnewstitle; ?>" onkeydown="return(postTitleErrorMsgDelete());"></td></tr>
        <tr style="background:#f2f2f2;"><td><b>Post Author</b><p style="float:right; color:red;" id="postAuthorErrorMsg"></p></td></tr>
        <tr><td><input type="text" class="form-control" name="postAuthor" value="<?php echo $db_worldnewsauthor; ?>" onkeydown="return(postAuthorErrorMsgDelete());"></td></tr>
       ...
       <input type="file" id="fileChooser" name="fileToUpload" multiple accept='image/*'   onclick="return(postImageErrorMsgDelete());"></td></tr>
        <tr><td><p>Your Selected Image:</p>
            ...
            <?php 
            $selectImage = $target_dir . $db_worldnewsimages;
        ...
        <tr><td><textarea class="ckeditor" name="postContent"  onkeydown="return(postContentErrorMsgDelete());"><?php echo $db_worldnewscontent; ?></textarea></td></tr>
