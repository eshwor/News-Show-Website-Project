...
//insert data to database
    $sbUpdate = "UPDATE sidebarposts SET sidebarposttitle = '{$filter_sidebarTitle}', sidebarpostcontent = '{$filter_sidebarContent}' WHERE id = '{$editId}' "; 
    $UpdateResult = mysqli_query($connection,$sbUpdate);
    if($UpdateResult){
         echo "Updating Data, Please wait....."."&nbsp&nbsp"."<img  style='width:5%; height:6%;' src='../image/redirecting.gif'>";         
     }else{
         die("Failed Query!" . mysqli_error($connection));
     }
     //header("Location:addposts.php");
     header("refresh:5;../pages/sideBarPosts.php");
}else{
    unset($_POST);
}
?>
<?php
  //pull all the data from database for edit
if(isset($_GET['edit'])){
    $editId = $_GET['edit'];
    $editSql = "SELECT * FROM sidebarposts WHERE id = {$editId} ";
    $editResult = mysqli_query($connection,$editSql);
while($rows = mysqli_fetch_assoc($editResult)){
    $sbTitle = $rows['sidebarposttitle'];
    $sbContent = $rows['sidebarpostcontent'];
}
}
...
<br>
             <span class="label label-info">You are in Sidebar Edit Posts </span>
        ...
        <tr><td><input type="text" class="form-control" name="sbTitle" onkeydown="return(titleErrorMsgDelete());" value="<?php echo $sbTitle; ?>"></td></tr>
        <tr style="background:#f2f2f2;"><td><b>Content</b><p style="float:right; color:red;" id="contentErrorMsg"></p></td></tr>
         <tr><td><textarea rows="7" class="form-control" name="sbContent" onkeydown="return(contentErrorMsgDelete());"><?php echo $sbContent; ?></textarea></td></tr>
